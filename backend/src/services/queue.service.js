...
async function sendWithRetry(instanceId, instancePhone, jid, recipientName, message, originalId, sourceIp, imageUrl = null, maxAttempts = 3) {
  let lastErr;
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      await sendMessage(instanceId, jid, message, imageUrl);
...
try {
  ...
  const worker = new Worker(
    'wa-messages',
    async (job) => {
      const { instanceId, instancePhone, jid, recipientName, message, originalId, sourceIp, imageUrl } = job.data;
      console.log(`[queue:bullmq] [${instanceId}] Processing job ${job.id} → ${jid}`);
      await sendMessage(instanceId, jid, message, imageUrl ?? null);
      await addLog({ instanceId, instancePhone, id: originalId, recipientName, message, status: 'success', error: null, sourceIp });
    },
  ...
  enqueueMessage = async (instanceId, instancePhone, jid, recipientName, message, originalId, sourceIp, imageUrl = null) => {
    const job = await messageQueue.add('send', { instanceId, instancePhone, jid, recipientName, message, originalId, sourceIp, imageUrl });
    return job.id;
  };
  ...
  enqueueMessage = async (instanceId, instancePhone, jid, recipientName, message, originalId, sourceIp, imageUrl = null) => {
    setImmediate(() => sendWithRetry(instanceId, instancePhone, jid, recipientName, message, originalId, sourceIp, imageUrl).catch(() => {}));
    return `direct-${Date.now()}`;
  };
}
